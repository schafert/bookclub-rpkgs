...
## Introduction - preparing for submission 
...
- Include a `cran-comments.md` file tracked by git but ignored by `.Rbuildignore` (`usethis::use_cran_comments()`) to describe the results from an `R CMD CHECK` and the systems that it was run on (those on your system, `rhub` or GitHub actions)
    + two main sections are **Check results** and **Downstream dependencies**


## Test environments

CRAN runs on Windows, Mac OS X, Linux and Solaris. You don't need to test on all but you need to test on some (at least two) and mention in the comments which you used. 

Two ways to test on different platforms:

1. manually with `rhub::check()` and 
2. automatically with `usethis::use_github_action_check_standard()`
...
- prompts the user to do final checks including spelling, checking on `rhub`, committing all changes etc (best practices)
- allows you to add your own questions with `release_questions()`
...
If you get accepted, prepare for next version:

- success!
- when you did the submission, a `CRAN-SUBMISSION` or `CRAN-RELEASE` file was created. You don't need to commit this to GitHub. When it is accepted, run: `usethis::use_github_release()`. This creates a draft release on GitHub which you can then submit online.
